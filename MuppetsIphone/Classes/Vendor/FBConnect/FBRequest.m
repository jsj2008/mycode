...
+ (NSString*)serializeURL:(NSString *)baseUrl
  ...
  for (NSString* key in [params keyEnumerator]) {
    ...
    if (![key isEqualToString:@"limit"] && ![key isEqualToString:@"until"])
      [pairs addObject:[NSString stringWithFormat:@"%@=%@", key, escaped_value]];
    [escaped_value release];
  }
    
  // these values only work as end params - gaurav
  // facebook ignores these anywhere in between
  if ([params objectForKey:@"limit"]) {
    [pairs addObject:[NSString stringWithFormat:@"%@=%@", @"limit", [params objectForKey:@"limit"]]];
  }

  if ([params objectForKey:@"until"]) {
    [pairs addObject:[NSString stringWithFormat:@"%@=%@", @"until", [params objectForKey:@"until"]]];
  }
...
- (void)failWithError:(NSError *)error {
  ...
  } else if (self.facebook.errorBlock){
      self.facebook.errorBlock(error);
  }
}
...
- (void)handleResponseData:(NSData *)data {
  ...
  if ([_delegate respondsToSelector:@selector(request:didLoad:)] ||
      ...
      self.facebook.responseBlock ||
      self.facebook.errorBlock
     ) {
    NSError* error = nil;
    ...
    } else if (self.facebook.responseBlock){
        self.facebook.responseBlock((result == nil ? data : result));
    }
